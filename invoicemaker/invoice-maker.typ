...
#let languages = (
    ...
    de: (
      ...
      customer-id: "Kundennummer", // ← NEU in de:
      items: "Leistungen",
      ...
      total: "Gesamt",
      due-text: val =>
        [Bitte überweisen Sie den Betrag bis zum *#val* auf folgendes Konto:],
      owner: "Inhaber",
      iban: "IBAN",
      bank-name: "Bank"
    ),
...
#let invoice(
  ...
  if type(issuing-date) == str {
    parse-date(issuing-date)
  } else {
    issuing-date
  }
} else {
  ...
  if type(delivery-date) == str {
    parse-date(delivery-date)
  } else {
    delivery-date
  }
} else {
  datetime.today() // oder TODO
}
  ...
  align(center,
    table(
     ...
     [#t.issuing-date:], [#text(weight: "bold")[#issuing-date.display("[day].[month].[year]")]],
[#t.delivery-date:], [#text(weight: "bold")[#delivery-date.display("[day].[month].[year]")]],
  [#t.customer-id:], [#text(weight: "bold")[#customer-id]], 
...
#{ if biller.vat-id != none { "USt-IdNr.: " + biller.vat-id } else { text(" ") } }
...
table(
  columns: (auto, 2fr, auto, auto, auto), // Beschreibung = 2fr → mehr Platz → Umbruch möglich
  align: (col, row) =>
    if row == 0 {
  ...
  ..items
    ...
    .map(((index, row)) => (
      ...
      row.description, // KEIN box(), KEIN wrap – das geht durch 2fr automatisch!
      str(row.at("quantity", default: 1)),
      str(add-zeros(cancel-neg *
        row.at("price", default: calc.round(hourly-rate * (row.at("dur-min", default: 0) / 60), digits: 2))
      )),
...
if cancellation-id == none {
  let due-date = if due-date != none { due-date } else {
    (parse-date(issuing-date) + duration(days: 14))
      .display("[year]-[month]-[day]")
  }

  // -- Untrennbarer Zahlungsblock (bricht nicht zwischen Seiten) --
  block(breakable: false)[
    #(t.due-text)(parse-date(due-date).display("[day].[month].[year]"))
